...
contract PubkeySetStorage is StafiBase, IPubkeySetStorage {
    ...
    function getItem(bytes32 _key, uint256 _index) override external view returns (bytes memory) {
        return getBytes(keccak256(abi.encodePacked(_key, ".item", _index)));
    }

    // The index of an item in a set
    // Returns -1 if the value is not found
    function getIndexOf(bytes32 _key, bytes calldata _value) override external view returns (int256) {
        return int256(getUint(keccak256(abi.encodePacked(_key, ".index", _value)))) - 1;
    }

    // Add an item to a set
    // Requires that the item does not exist in the set
    function addItem(bytes32 _key, bytes memory _value) override external onlyLatestContract("pubkeySetStorage", address(this)) onlyLatestNetworkContract {
    ...
    function removeItem(bytes32 _key, bytes calldata _value) override external onlyLatestContract("pubkeySetStorage", address(this)) onlyLatestNetworkContract {
